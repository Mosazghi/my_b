...
static void CalculateCurrentTimeString(void) {
  ...
  if (!SDL_GetCurrentTime(&ticks) || !SDL_TimeToDateTime(ticks, &dt, true)) {
    SDL_snprintf(current_time, sizeof(current_time),
    ...
    SDL_snprintf(current_time, sizeof(current_time),
                 "%s, %s %d, %d   %02d:%02d:%02d", day[dt.day_of_week],
                 month[dt.month - 1], dt.day, dt.year, dt.hour, dt.minute,
                 dt.second);
  }
}
...
SDL_AppResult SDL_AppInit(void** appstate, int argc, char* argv[]) {
  SDL_SetAppMetadata("Example Misc Clipboard", "1.0",
  ...
  if (!SDL_CreateWindowAndRenderer("examples/misc/clipboard", 640, 480,
  ...
  copybuttonrect.w =
      (float)((SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE * SDL_strlen(copybuttonstr)) +
              10);
  ...
  pastebuttonrect.w = (float)((SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE *
                               SDL_strlen(pastebuttonstr)) +
                              10);
...
static void RenderPastedText(void) {
  ...
  if (str) {
    ...
    const size_t max_chars_per_line =
        (size_t)(w / SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE);
    char* newline;
    size_t slen;
    char ch;
...
SDL_AppResult SDL_AppIterate(void* appstate) {
  ...
  x = currenttimerect.x +
      ((currenttimerect.w -
        (SDL_DEBUG_TEXT_FONT_CHARACTER_SIZE * SDL_strlen(current_time))) /
       2.0f);
  ...
  SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
  SDL_RenderDebugText(renderer, copybuttonrect.x + 5, copybuttonrect.y + 5,
                      copybuttonstr);
  ...
  SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);
  SDL_RenderDebugText(renderer, pastebuttonrect.x + 5, pastebuttonrect.y + 5,
                      pastebuttonstr);
